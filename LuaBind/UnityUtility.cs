...
public class UnityUtility : SingletonMonoBehaviour<UnityUtility> {
	...
	Dictionary<string, string> mLuaCallUpdateMap = null;
...
//		gcHandle = GCHandle.Alloc(LuaUnityLoadLevel);
	...
	int UnityLoadPrefab(IntPtr luaState)
	{
	...
	int UnityLoadLevel(IntPtr luaState)
	{
	...
	int UnityDebugLog(IntPtr luaState)
	{
	...
	// Luaファイル読み込み。他のLuaファイルから別のLuaファイルの処理を呼び出したい時に使う
	int UnityLoadLuaFile(IntPtr luaState)
	{
	...
	// Lua側のCavManagerの初期化
	int UnityInitCsvManager(IntPtr luaState)
	{
	...
	int UnityLoadCsv(IntPtr luastate)
	{
	...
	int UnityCallLuaFunction(IntPtr luastate)
	{
	...
	// Updateを呼び出したいLuaスクリプトの登録
	int UnityAddCallUpdateScript(IntPtr luastate)
	{
	...
	// どこでも使うUnityの関数を、Luaに登録する
	int UnityBindCommonFunction(IntPtr luastate)
	{
	...
	public void BindCommonFunction(string scriptName)
	{
		// シーン切り替え
